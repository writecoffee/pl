...
fun number_in_months_challenge(date_list : (int * int * int) list, month_list : int list) =
	...
	if null date_list then
	    ...
	    number_in_months(hd date_list, month_list) + number_in_months_challenge(tl date_list, month_list)
    end
...
fun reasonable_date(date : int * int * int) =
	...
	val d = #3 date
    in
	y > 0 andalso (m > 0 andalso m < 13) andalso d > 0 andalso
	((is_leap_year(y) andalso d <= get_nth(days_leap_list, m)) orelse d <= get_nth(days_list, m))
    end
